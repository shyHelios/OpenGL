...
TestMaterial::TestMaterial(const std::string& InDisplayName) :
    Test(InDisplayName)
{
    // 开启深度测试
    ...
    std::unordered_map<unsigned int, std::string> boxShaderFiles{
            {GL_VERTEX_SHADER,   "resources/material/shaders/material.vert"         },
            {GL_FRAGMENT_SHADER, "resources/material/shaders/material.frag"}
    };
    BoxShader = std::make_unique<Shader>(boxShaderFiles);

    std::unordered_map<unsigned int, std::string> lightShaderFiles{
            {GL_VERTEX_SHADER,   "resources/material/shaders/light.vert"},
            {GL_FRAGMENT_SHADER, "resources/material/shaders/light.frag"}
    };
...
void TestMaterial::OnUpdate(float delta_time)
{
